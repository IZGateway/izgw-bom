...
`izgw-bom` imports `spring-boot-dependencies` (Spring Boot 3.5.14), which pins `io.opentelemetry:opentelemetry-bom` to **1.49.0**. `ca.uhn.hapi.fhir:hapi-fhir-base:8.10.0` requests OpenTelemetry **1.60.1**, but Spring's managed BOM forces it back down to 1.49.0 in every consumer (confirmed in `v2tofhir`: `opentelemetry-api:jar:1.49.0:compile (version managed from 1.60.1)`). Consumers should run the OTel version HAPI was actually built against, and the fix belongs centrally in the BOM so all consumers inherit it.

## What Changes

- Add an `<opentelemetry.version>1.60.1</opentelemetry.version>` property to the `<properties>` block.
...
- **All consumers** (e.g. `v2tofhir`) inherit OTel 1.60.1 once the updated `1.9.0-SNAPSHOT` is installed/published; no consumer pom changes required.
- **Dependencies**: `io.opentelemetry:*` bumped 1.49.0 → 1.60.1 (semver-compatible within 1.x; low risk).
