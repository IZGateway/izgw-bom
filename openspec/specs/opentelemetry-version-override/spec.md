# Spec: OpenTelemetry Version Override

## Purpose

Defines how `izgw-bom` centrally pins the OpenTelemetry version for all consumers,
overriding the versions transitively supplied by the imported `spring-boot-dependencies`
BOM and by `hapi-fhir-base`. This ensures a single, deliberately-chosen OpenTelemetry
version is resolved consistently and is CVE-scanned by CI.
